...
export class ResourceComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.dataDrivenFilterEvent.pipe(first()).subscribe((filters: any) => {
  ...
  private fetchPageData() {
    ...
    this.pageApiService.getPageData(option).subscribe(
      data => {
  ...
  private prepareCarouselData(sections = []) {
    const {
      ...
      dynamicFields,
      slickSize
    } = this.configService.appConfig.Library;
  ...
  public viewAll(event) {
    ...
    searchQuery.request.filters.softConstraintsFilter = JSON.stringify(
      softConstraintsFilter
    );
    searchQuery.request.filters.defaultSortBy = JSON.stringify(
